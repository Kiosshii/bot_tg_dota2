...
@dp.message_handler(commands=['IO', 'io'])  # выбор героя io независимо от регистра
...
async def send_io_photo(message: types.Message):
    photo_path = 'wisp.jpg'
    caption = '''Io — это герой в игре Dota,
который представляет собой совокупность всех сил притяжения и отторжения в материальном поле.
Может выбрать один из двух аспектов: Атакующий — Kritzkrieg,
с которым Overcharge увеличивает скорость атаки и урон заклинаний обоим связанным героям.
Защитный — Medigun, предоставляющий броню и
сопротивление магии соответственно. Ультимативная способность Io — Relocate,
которая даёт возможность ему и связанному с ним союзнику перенестись в любую точку карты.
Через несколько секунд Io телепортируется обратно, союзник же вернётся, только если он всё ещё привязан!'''

    # Создаем клавиатуру с кнопкой "Далее"
    keyboard = types.InlineKeyboardMarkup()
    next_button = types.InlineKeyboardButton(text="Далее к Techies", callback_data='next_techies')
...
@dp.callback_query_handler(lambda c: c.data == 'next_techies')
async def process_next_info(callback_query: types.CallbackQuery):
    await bot.answer_callback_query(callback_query.id)
    await send_techies_photo(callback_query.message.chat.id)
...
async def send_techies_photo(chat_id):
    photo_path = 'This.jpg'
    caption = '''Techies
вспомните длинные поля в Dota, так вот они все были заминированы.
Окунитесь в мир минных полей и самолетов-камикадзен на одном персонаже.
Хардлайн (3 позиция); Саппорт (4 позиция); Саппорт (5 позиция).'''
